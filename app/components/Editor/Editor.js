...
import { CancelSVGIcon, ChevronRightSVGIcon, MoreHorizSVGIcon, SaveSVGIcon } from "@react-md/material-icons"
import { useSelector } from "react-redux"
import { Context } from "../MainWrapper/Context"
import DropDownEdit from "../DropDownEdit/DropDownEditArticle/DropDownEditArticle"
...
const Editor = (props) => {
  ...
  const onSaveHandler = async (editorInstance, type) => {
    ...
    try {
      ...
      props.onSave(blogData, title, description, type)
      disable()
    } catch (err) {
      console.log(err)
    }
        ...
        <div className="overflow-y-scroll p-8 w-3/5">
          {body && <div>
            <nav className="flex justify-between border-b-2 border-gray-200">
              ...
              {!modeCreate && <DropDownEdit type="article" />}
            </nav>
            {modeCreate ?
              <section className="p-4">
                <div className="flex justify-between">
                  <div className="flex flex-col w-2/3">
                    <input className="fadeAnimation  outline-none font-medium leading-tight text-4xl text-black" style={{ border: "0px" }}
                      idprefix="title-field"
                      onChange={(event) => setTitle(event.target.value)}
                      value={title}
                      placeholder="Заголовок"
                    />
                    <input className="fadeAnimation font-normal leading-tight outline-none text-2xl mb-8  text-black" style={{ border: "0px" }}
                      idprefix="desc-field"
                  ...
                  <div className="flex justify-between gap-5 w-1/3" style={{ width: '30%', textAlign: 'center', maxHeight: "35px" }}>
                    <button
              ...
              <>
                <section className="p-4">
                  {editArticle ?
                    <div className="flex justify-between">
                      <div className="flex flex-col w-2/3 ">
                        <input onChange={(event) => setTitle(event.target.value)} className="fadeAnimation  outline-none font-medium leading-tight text-4xl text-black" value={title} placeholder={title} />
                        <input onChange={(event) => setDescription(event.target.value)} value={description} placeholder={description} className="fadeAnimation font-normal leading-tight outline-none text-2xl mb-8  text-black" />
                      </div>
                      <div className="flex justify-between gap-5 w-1/3" style={{ maxHeight: "35px" }}>
                        <button
                          onClick={() => onCancelHandler()}
                          className="flex items-center justify-around w-92-p rounded-lg p-3"
                          style={{ backgroundColor: "#f6f6f6" }}
                    ...
                    :
                    <div>
                      <div className="fadeAnimation font-medium leading-tight text-4xl text-black">{article?.title}</div>
                      <div className="fadeAnimation font-normal leading-tight text-2xl mb-8  text-black">{article?.description}</div>
                    </div>}
                    ...
                    <div className=" flex gap-10 items-center">
                      ...
                      <div className="flex flex-col gap-5 text-black text-base font-normal">
                        <div className="flex gap-2 items-center">
                          <div>
                            <img className="w-6 h-6 rounded-sm" src={`http://192.168.0.203:8787/${article?.user?.foto_url}`} />
                          </div>
                          <div>
                            {article?.user?.name}
                          </div>
